...
bool loadOBJ(const char* path, std::vector<glm::vec3>& out_vertices, std::vector<glm::vec2>& out_uvs, std::vector<glm::vec3>& out_normals) {
	...
	if(file == NULL) {
		std::cerr << "[File Error] Can't open OBJ file " << path << std::endl;
		//exit(3);
		return false;
	}
	...
	while(1) {
		...
		if(strcmp(lineHeader, "v") == 0) {
			...
			if(matches != 9) {
				std::cerr << "[OBJ Error] Error reading file, wrong face arguments" << std::endl;
				fclose(file);
				return false;
			}
...
ObjMesh::ObjMesh(const char* path) : Geometry() {
	...
	m_vertices = (float*)malloc(sizeof(float) *  3 * o_vertices.size());
